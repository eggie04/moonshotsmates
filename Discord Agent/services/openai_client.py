...
class AiHelper:
    ...
    def generate_business_ideas(self, model_override: str) -> str:
        prompt = (
            ...
            "Generate 8-12 novel startup ideas across multiple sectors where software and data can create a real "
            "advantage. AI can be used as a tool, but do not make every idea AI-native.\n"
            "Portfolio mix requirements:\n"
            "- At most 40% of ideas can be AI-native products\n"
            "- Include at least one idea each in B2B SaaS, consumer, and a real-world/operational sector "
            "(for example: logistics, construction, healthcare ops, climate/energy, manufacturing, education, or "
            "local services)\n"
            "- Include some ideas where AI is only a supporting capability rather than the core product\n"
            "Prioritize:\n"
            "- painful, specific problems with clear willingness to pay\n"
            "- strong distribution or wedge opportunities\n"
            "- realistic first customers and fast validation paths\n"
            ...
            "- Ensure the final set is diverse in sector and business model; avoid repeating the same AI-agent pattern\n"
